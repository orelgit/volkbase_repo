...
@section('content')
    ...
    <div class="d-flex justify-content-between mb-5 mt-4 ">
        <h1 class="h3 text-primary">تعديل مقالة</h1>
    ...
    <div class="container">
        <div class="row d-flex justify-content-center mb-3 ">
            <div class="card bg-white col-10 shadow">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin-UpdateBlog-work', $blog->b_id) }}"
                        ...
                        <div class="mb-3">
                            <div class="col-lg-6 col-md-12">
                                ...
                                <input value="{{ asset('storage/' . $blog->b_img) }}" class="form-control bg-light"
                                    type="file" name="b_img" id="blog-img">
                                <img class="mt-2  rounded border border-primary"
                                    src="{{ asset('storage/' . $blog->b_img) }}" alt="" style="width: 250px">
                                @error('b_img')
                                    <h6 class="mt-2 text-danger"> <i class="fas fa-times"></i> {{ $message }}</h6>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="b_blog" class="form-control bg-light"
                                id="blog-body">{{ $blog->b_blog }}</textarea>
                            @error('b_blog')
                                <h6 class="mt-2 text-danger"> <i class="fas fa-times"></i> {{ $message }}</h6>
                            @enderror
                        </div>
